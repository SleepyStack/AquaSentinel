...
// Handle report submission with JSON only
async function handleReportSubmit(event) {
    ...
    // Collect all form values into JSON
    const report = {
    ...
    try {
        ...
        const response = await fetch("http://localhost:8080/api/reports", {
            ...
            headers: {
                ...
                // Basic Auth header (if enabled in backend)
                "Authorization": "Basic " + btoa("yourEmail@example.com:yourPassword")
            },
        ...
        spinnerText.textContent = "Submission failed ❌";
        spinnerText.classList.add("failure");
        console.error("Error:", error);
    }

    // Reset spinner after 3s
    setTimeout(() => {
